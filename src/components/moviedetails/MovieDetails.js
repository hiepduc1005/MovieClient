...
const MovieDetails = ({ user, token }) => {
  ...
  const handleAddOrRemoveMovieFromWatchList = async (watchlistId, movieId) => {
    if (user && token) {
      try {
        ...
        if (data) {
          setNotificationMessage(newIsInWatchList ? 'Added to Watch Later' : 'Removed from Watch Later')
          setShow(true)
        }
      ...
      <div className='movie-details-info'>
        <div className='movie-details-content'>
          ...
          <div className='movie-details-btn'>
            ...
            <div className='btn btn-watchlater' onClick={() => handleAddOrRemoveMovieFromWatchList(user?.watchList?.id, movie?.id)}>
              {isInWatchList ?
